...
export function assertSafeArchiveEntries(entries: Array<string | ArchiveEntryInfo>): void {
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      throw new Error(`Unsafe Axis CLI archive entry: ${entryName}`);
    }
    if (seen.has(normalized)) {
      throw new Error(`Duplicate Axis CLI archive entry: ${entryName}`);
    }
    if (entryType && entryType !== 'file' && entryType !== 'directory') {
      throw new Error(`Unsafe Axis CLI archive entry type for ${entryName}.`);
    }
    seen.add(normalized);
  }
}
...
export async function validateExtractedAxisCliPayload(input: { root: string; executableName: string }): Promise<void> {
  ...
  if (!executableStat?.isFile()) {
    throw new Error(`Extracted Axis CLI payload is missing executable ${input.executableName}.`);
  }
  const skillsStat = await stat(skills).catch(() => undefined);
  if (!skillsStat?.isDirectory()) {
    throw new Error('Extracted Axis CLI payload is missing skills/.');
  }
  await access(skills);
}
