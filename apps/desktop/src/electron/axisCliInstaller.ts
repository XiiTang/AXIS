import { randomUUID } from 'node:crypto';
import { mkdir, readFile, rename, rm, symlink, writeFile } from 'node:fs/promises';
import { dirname, join } from 'node:path';
import { tmpdir } from 'node:os';
import type {
  AxisCliInstallResult,
  AxisCliManualCommand,
  AxisCliPathRepairResult,
  AxisCliSkillsStatus,
  AxisCliSkillsSyncResult,
  AxisCliStatus
} from '@axis/app-protocol';
import {
  checksumForAsset,
  extractAxisCliArchive,
  parseSha256Manifest,
  sha256File,
  validateExtractedAxisCliPayload
} from './axisCliArchive.js';
import {
  axisCliAssetName,
  axisCliChecksumUrl,
  axisCliExecutableName,
  axisCliManagedPaths,
  axisCliReleaseUrl
} from './axisCliPaths.js';
import {
  getAxisCliStatus,
  manualInstallCommand,
  pathRepairCommand,
  repairAxisCliPath
} from './axisCliStatus.js';
import { runAxisCli, type AxisCliRunResult } from './axisCliProcess.js';

export interface AxisCliInstaller {
  getStatus(): Promise<AxisCliStatus>;
  install(): Promise<AxisCliInstallResult>;
  update(): Promise<AxisCliInstallResult>;
  repairPath(): Promise<AxisCliPathRepairResult>;
  syncSkills(force?: boolean): Promise<AxisCliSkillsSyncResult>;
  getManualInstallCommand(): Promise<AxisCliManualCommand>;
}

export interface AxisCliInstallerInput {
  desktopVersion: string;
  userHome: string;
  platform?: NodeJS.Platform;
  arch?: NodeJS.Architecture;
  envPath?: string;
  shell?: string;
  fetchAsset?: (url: string, destination: string) => Promise<void>;
  extractArchive?: (input: { archivePath: string; destinationDir: string; platform: NodeJS.Platform }) => Promise<void>;
  repairPath?: () => Promise<void>;
  runAxis?: (axisPath: string, args: string[]) => Promise<AxisCliRunResult>;
}

export function createAxisCliInstaller(input: AxisCliInstallerInput): AxisCliInstaller {
  const platform = input.platform ?? process.platform;
  const arch = input.arch ?? process.arch;
  const runAxis = input.runAxis ?? runAxisCli;
  const manual = () => manualInstallCommand({ version: input.desktopVersion, platform, arch });
  const managedPaths = () => axisCliManagedPaths({ userHome: input.userHome, version: input.desktopVersion, platform });
  const repairInput = () => ({
    userHome: input.userHome,
    platform,
    ...(input.shell ? { shell: input.shell } : {})
  });

  const getStatusForEnvPath = (envPath?: string) => getAxisCliStatus({
    desktopVersion: input.desktopVersion,
    userHome: input.userHome,
    platform,
    arch,
    runAxis,
    ...(envPath !== undefined ? { envPath } : {}),
    ...(input.shell ? { shell: input.shell } : {})
  });
  const getStatus = () => getStatusForEnvPath(input.envPath);

  async function install(): Promise<AxisCliInstallResult> {
    const assetName = axisCliAssetName({ version: input.desktopVersion, platform, arch });
    const tempRoot = join(tmpdir(), `axis-cli-${process.pid}-${Date.now()}`);
    const archivePath = join(tempRoot, assetName);
    const checksumPath = join(tempRoot, 'axis_SHA256SUMS');
    const paths = managedPaths();
    const stagedInstallDir = join(dirname(paths.installDir), `.${input.desktopVersion}.${process.pid}.${randomUUID()}.install`);
    try {
      await mkdir(tempRoot, { recursive: true });
      const fetchAsset = input.fetchAsset ?? downloadFile;
      await fetchAsset(axisCliReleaseUrl({ version: input.desktopVersion, assetName }), archivePath);
      await fetchAsset(axisCliChecksumUrl(input.desktopVersion), checksumPath);
      const expectedChecksum = checksumForAsset(parseSha256Manifest(await readFile(checksumPath, 'utf8')), assetName);
      const actualChecksum = await sha256File(archivePath);
      if (actualChecksum !== expectedChecksum) {
        throw new Error(`Checksum mismatch for ${assetName}.`);
      }

      await rm(stagedInstallDir, { recursive: true, force: true });
      await mkdir(stagedInstallDir, { recursive: true });
      await (input.extractArchive ?? extractAxisCliArchive)({ archivePath, destinationDir: stagedInstallDir, platform });
      await validateExtractedAxisCliPayload({ root: stagedInstallDir, executableName: axisCliExecutableName(platform) });

      const stagedExecutablePath = join(stagedInstallDir, axisCliExecutableName(platform));
      const versionResult = await runAxis(stagedExecutablePath, ['--version']);
      if (versionResult.exitCode !== 0 || parseAxisVersion(versionResult.stdout) !== input.desktopVersion) {
        throw new Error(`Axis CLI version verification failed: ${versionResult.stderr || versionResult.stdout}`);
      }

      await mkdir(dirname(paths.installDir), { recursive: true });
      await rm(paths.installDir, { recursive: true, force: true });
      await rename(stagedInstallDir, paths.installDir);
      await activateShim(paths, platform);

      let pathRepairFailed = false;
      try {
        await (input.repairPath ?? (() => repairAxisCliPath(repairInput())))();
      } catch {
        pathRepairFailed = true;
      }

      const skillsResult = await runAxis(paths.shimPath, ['skills', 'sync']);
      const status = await getStatusForEnvPath(pathRepairFailed
        ? input.envPath
        : envPathWithManagedBin(paths.binDir, input.envPath ?? process.env.PATH ?? '', platform));
      const statusWithSkills = withSkillsStatus(status, skillsResult.exitCode === 0
        ? skillsStatusFromCliStatus(status)
        : { kind: 'error', code: 'skills_sync_failed', message: skillsResult.stderr || 'Axis Skills sync failed.' });

      if (pathRepairFailed && 'cliVersion' in statusWithSkills) {
        return {
          ok: true,
          status: {
            kind: 'installed_but_not_on_path',
            desktopVersion: input.desktopVersion,
            cliVersion: statusWithSkills.cliVersion,
            managedPath: paths.shimPath,
            repairCommand: pathRepairCommand({ userHome: input.userHome, platform }),
            skills: skillsStatusFromCliStatus(statusWithSkills)
          }
        };
      }
      return { ok: true, status: statusWithSkills };
    } catch (error) {
      return {
        ok: false,
        status: {
          kind: 'error',
          desktopVersion: input.desktopVersion,
          code: 'axis_cli_install_failed',
          message: error instanceof Error ? error.message : String(error),
          manualCommand: manual().command
        }
      };
    } finally {
      await rm(stagedInstallDir, { recursive: true, force: true }).catch(() => undefined);
      await rm(tempRoot, { recursive: true, force: true }).catch(() => undefined);
    }
  }

  return {
    getStatus,
    install,
    update: install,
    repairPath: async () => {
      const paths = managedPaths();
      await (input.repairPath ?? (() => repairAxisCliPath(repairInput())))();
      return { ok: true, status: await getStatusForEnvPath(envPathWithManagedBin(paths.binDir, input.envPath ?? process.env.PATH ?? '', platform)) };
    },
    syncSkills: async (force = false) => {
      const paths = managedPaths();
      const result = await runAxis(paths.shimPath, ['skills', 'sync', ...(force ? ['--force'] : [])]);
      if (result.exitCode !== 0) {
        return { ok: false, status: { kind: 'error', code: 'skills_sync_failed', message: result.stderr || 'Axis Skills sync failed.' } };
      }
      return { ok: true, status: skillsStatusFromCliStatus(await getStatus()) };
    },
    getManualInstallCommand: async () => manual()
  };
}

async function activateShim(
  paths: ReturnType<typeof axisCliManagedPaths>,
  platform: NodeJS.Platform
): Promise<void> {
  await mkdir(paths.binDir, { recursive: true });
  await rm(paths.shimPath, { force: true });
  if (platform === 'win32') {
    await writeFile(paths.shimPath, `@echo off\r\n"${paths.executablePath}" %*\r\n`, 'utf8');
    return;
  }
  await symlink(paths.executablePath, paths.shimPath);
}

async function downloadFile(url: string, destination: string): Promise<void> {
  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(`Failed to download ${url}: ${response.status}`);
  }
  await writeFile(destination, Buffer.from(await response.arrayBuffer()));
}

function parseAxisVersion(stdout: string): string | undefined {
  return /(\d+\.\d+\.\d+)/.exec(stdout)?.[1];
}

function skillsStatusFromCliStatus(status: AxisCliStatus): AxisCliSkillsStatus {
  return 'skills' in status ? status.skills : { kind: 'not_checked' };
}

function withSkillsStatus(status: AxisCliStatus, skills: AxisCliSkillsStatus): AxisCliStatus {
  if (!('skills' in status)) return status;
  return { ...status, skills };
}

function envPathWithManagedBin(binDir: string, envPath: string, platform: NodeJS.Platform): string {
  const pathDelimiter = platform === 'win32' ? ';' : ':';
  const entries = envPath.split(pathDelimiter).filter(Boolean);
  const nextEntries = [
    binDir,
    ...entries.filter((entry) => !samePathEntry(entry, binDir, platform))
  ];
  return nextEntries.join(pathDelimiter);
}

function samePathEntry(left: string, right: string, platform: NodeJS.Platform): boolean {
  return platform === 'win32'
    ? left.toLowerCase() === right.toLowerCase()
    : left === right;
}
