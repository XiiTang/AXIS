...
export type DebruteCliErrorCode =
  | 'invalid_command'
  | 'invalid_argument'
  | 'missing_argument'
  | 'invalid_input'
...
export type DebruteCliExitCode = 0 | 1 | 2 | 3 | 4 | 5;

export class DebruteCliError extends Error {
  constructor(
...
export function isDebruteCliError(error: unknown): error is DebruteCliError {
  return error instanceof DebruteCliError;
}

export function exitCodeForCliError(error: unknown): DebruteCliExitCode {
...
export function normalizeServiceErrorCode(code: string): DebruteCliErrorCode {
  if (isDebruteCliErrorCode(code)) {
    return code;
  }
  if (
...
export function primitiveErrorFields(fields: unknown): DebruteCliError['fields'] {
  return primitiveFields(fields);
}

export function projectLoadCliError(error: unknown, projectRoot: string): DebruteCliError {
...
function isDebruteCliErrorCode(code: string): code is DebruteCliErrorCode {
  return [
    'invalid_command',
    'invalid_argument',
    'missing_argument',
